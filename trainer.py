...
class Trainer:
    ...
    def run_epoch(self, split, epoch, set_name, grad):
        ...
        for s in split:
            ...
            predictions, nodes_embs = self.predict(s.label_sp['idx'],
                                                   s.idx,
                                                   set_name)
    ...
    def predict(self, node_indices, cut_time, set_name):
        if set_name == 'TEST' or 'VALID':
            self.gcn.eval()
        else:
            self.gcn.train()
        nodes_embs = self.gcn(np.arange(self.tasker.data.num_nodes), np.ones(self.tasker.data.num_nodes)*cut_time.numpy())
        predict_batch_size = 100000
        gather_predictions = []
        for i in range(1 + (node_indices.size(1) // predict_batch_size)):
            cls_input = self.gather_node_embs(nodes_embs,
                                              node_indices[:,  i*predict_batch_size:(i+1)*predict_batch_size])
            predictions = self.classifier(cls_input.cuda())
