...
def build_adj_list(dataset):
    adj_list = [[] for _ in range(dataset.num_nodes+1)]
    for src, dst, ts in dataset.edges['idx']:
        adj_list[src].append((dst, 0, ts))
        adj_list[dst].append((src, 0, ts))
    return adj_list
