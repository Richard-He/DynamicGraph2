...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, feat_dim, num_node, num_hist_steps, n_feat=None, e_feat=None,
                 attn_mode='prod', use_time='time', agg_method='attn',
                 num_layers=2, n_head=4, null_idx=0, drop_out=0.1, seq_len=None):
        ...
        if n_feat or e_feat:
            ...
            self.n_feat_th = torch.nn.Parameter(torch.zeros(num_node, feat_dim))
            self.e_feat_th = torch.nn.Parameter(torch.zeros(1, feat_dim))
            self.edge_raw_embed = torch.nn.Embedding.from_pretrained(self.e_feat_th, padding_idx=0, freeze=True)
            self.node_raw_embed = torch.nn.Embedding.from_pretrained(self.n_feat_th, padding_idx=0, freeze=True)
        self.feat_dim = feat_dim
    ...
    def forward(self, node_ids, cut_time_l):
        src_embed = self.tem_conv(node_ids, cut_time_l, curr_layers=self.num_layers)
        return src_embed
